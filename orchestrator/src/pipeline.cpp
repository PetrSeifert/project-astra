...
void UserPipeline::process_audio(const std::vector<int16_t> & /*pcm*/,
  ...
  nlohmann::json llm_req = {{"utterance",
                             {{"user_id", utt.user_id},
                              {"t0", utt.t0},
                              {"t1", utt.t1},
                              {"text", utt.text}}}};
...
void PipelineManager::on_audio_chunk(
    const std::string &user_id, const std::vector<int16_t> &pcm,
    std::function<void(const std::vector<int16_t> &)> send_audio) {
